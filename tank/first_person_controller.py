...
class FirstPersonController(Entity):
    def __init__(self, **kwargs):
        ...
        if self.gravity:
            ray = raycast(
                self.world_position + (0, self.height, 0), self.down, ignore=(self,)
            )
            if ray.hit:
    ...
    def update(self):
        ...
        head_ray = raycast(
            self.position + Vec3(0, self.height - 0.1, 0),
            self.direction,
            ignore=(self,),
            distance=0.5,
            debug=False,
        ...
        if self.gravity:
            ...
            if ray.distance <= self.height + 0.1:
                ...
                if (
                    ray.world_normal.y > 0.7 and ray.world_point.y - self.world_y < 0.5
                ):  # walk up slope
...
if __name__ == "__main__":
    ...
    gun.on_click = Sequence(
        Func(setattr, gun, "parent", camera), Func(setattr, player, "gun", gun)
    )
    ...
    hookshot_target = Button(
        parent=scene, model="cube", color=color.brown, position=(4, 5, 5)
    )
    hookshot_target.on_click = Func(
        player.animate_position,
        hookshot_target.position,
        duration=0.5,
        curve=curve.linear,
    )
    ...
    def input(key):
        if key == "left mouse down" and player.gun:
            ...
            bullet = Entity(parent=gun, model="cube", scale=0.1, color=color.black)
            bullet.world_parent = scene
            bullet.animate_position(
                bullet.position + (bullet.forward * 50), curve=curve.linear, duration=1
            )
